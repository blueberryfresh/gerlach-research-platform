        ...
        if task_responses:
            ...
            for task_response in task_responses:
                ...
                # Check if Popcorn Brain
                    ...
                    md += "#### Self-Assessment Ratings (1-7 scale)\n\n"
                    md += f"- **Originality:** {task_response.originality_rating}/7\n"
                    md += f"- **Flexibility:** {task_response.flexibility_rating}/7\n"
                    md += f"- **Elaboration:** {task_response.elaboration_rating}/7\n"
                    md += f"- **Fluency:** {task_response.fluency_rating}/7\n\n"
                    
                    md += "#### Automated Metrics from Dialogue\n\n"
                    md += f"- **Unique Ideas Generated:** {task_response.unique_ideas}\n"
                    md += f"- **Alternative Approaches:** {task_response.alternative_approaches}\n"
                    md += f"- **Detail Instances:** {task_response.detail_instances}\n"
                    md += f"- **Total Ideas:** {task_response.total_ideas}\n"
                    md += f"- **Ideas per Minute:** {task_response.ideas_per_minute:.2f}\n\n"
                    
                    # Comparison table
                    md += "#### Self-Assessment vs Computed Metrics\n\n"
                    md += "| Dimension | Self-Rating | Computed Count |\n"
                    md += "|-----------|-------------|----------------|\n"
                    if task_response.originality:
                        md += f"| Originality | {task_response.originality_rating}/7 | {task_response.originality.computed_count} unique ideas |\n"
                    if task_response.flexibility:
                        md += f"| Flexibility | {task_response.flexibility_rating}/7 | {task_response.flexibility.computed_count} alternatives |\n"
                    if task_response.elaboration:
                        md += f"| Elaboration | {task_response.elaboration_rating}/7 | {task_response.elaboration.computed_count} details |\n"
                    if task_response.fluency:
                        md += f"| Fluency | {task_response.fluency_rating}/7 | {task_response.fluency.computed_count} total ideas |\n"
                    md += "\n"
