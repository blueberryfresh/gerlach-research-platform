...
EN = {
    ...
    # ── Big5 Assessment ───────────────────────────────────────────────────────
    "big5_header": "📋 Personality Assessment",
    "big5_info": (
        "This assessment is a core part of our study. Understanding your personality helps us "
        "explore how different people collaborate with AI — your responses allow us to draw "
        "meaningful scientific conclusions about human-AI interaction. Thank you for contributing "
        "to this research."
    ),
    "big5_instructions": (
    ...
    # ── Post-Task Survey ──────────────────────────────────────────────────────
    "survey_header":      "Post-Task Survey",
    "survey_instructions": (
...
KO = {
    ...
    "reg_intro": (
        "참여해 주셔서 감사합니다. 이 연구에서 귀하는 다음을 수행하게 됩니다:\n\n"
        "1. 몇 가지 설문지에 응답\n"
    ...
    "reg_id_help":           "이 ID는 귀하의 진행 상황을 추적하고 저장하는 데 사용됩니다.",
    "reg_consent":           "본 연구에 참여하는 것에 동의합니다",
    "reg_consent_help":      "귀하의 데이터는 익명으로 처리되며 연구 목적으로만 사용됩니다.",
    "reg_begin_btn":         "연구 시작",
    ...
    "big5_header": "📋 성격 평가",
    "big5_info": (
        "이 평가는 연구의 핵심 부분입니다. 귀하의 성격을 이해하면 "
        "다양한 사람들이 AI와 어떻게 협력하는지 탐구하는 데 도움이 됩니다 — "
        "귀하의 응답은 인간-AI 상호작용에 대한 의미 있는 과학적 결론을 도출할 수 있게 해줍니다. "
        "이 연구에 기여해 주셔서 감사합니다."
    ),
    "big5_instructions": (
        "다음 질문들은 귀하가 가진 성격 특성의 종류에 대해 알아보기 위해 설계되었습니다.\n"
        "사람들이 생각하거나, 느끼거나, 행동하는 방식을 설명하는 일련의 진술들이 제시됩니다.\n"
        "각 진술에 대해 귀하가 일반적으로 어떤지를 바탕으로 얼마나 동의하거나 동의하지 않는지를 표시하십시오 —\n"
        "옳거나 틀린 답은 없습니다.\n\n"
    ...
    "task_dial_warning":         "⚠️ **귀하와 LLM 파트너가 과제를 완전히 완료했을 때만 아래 버튼을 클릭하십시오!**",
    ...
    # ── Post-Task Survey ──────────────────────────────────────────────────────
    "survey_header":      "사후 과제 설문지",
    "survey_instructions": (
        "각 진술을 주의 깊게 읽고 귀하의 경험을 가장 잘 반영하는 응답을 선택하십시오. "
    ...
    "llm_language_instruction": (
        "IMPORTANT: You must respond exclusively in Korean (한국어) at all times, "
        "regardless of what language the participant uses."
    ),

    # ── Task content (Popcorn Brain — Korean) ─────────────────────────────────
    "task_popcorn_md": """\
참여해 주셔서 감사합니다. 아래 과제 설명을 주의 깊게 읽어 주십시오. 내용을 명확히 이해하기 위해 두 번 읽으시기를 권장합니다. 그런 다음 LLM과 대화를 시작하여 최적의 해결책을 함께 논의하고 협력하십시오. 합의에 도달한 후, 화면 하단의 '완료' 버튼을 클릭하여 세션을 종료하십시오. 귀하의 대화는 연구자들을 위해 저장될 것입니다.
...
Fresno 통합 교육구 교육감인 Johnson 씨는 디지털 기술, 특히 AI 기반 애플리케이션과 도구를 활용하여 학생 친화적이고 효과적인 학습 환경을 조성하는 새로운 교육구 전체 정책을 고민하고 있습니다. 주지사 사무실은 학생의 학업 성취도를 강화할 것이라고 믿는 "스마트 교실" 이니셔티브를 추진하고 있습니다.
...
이러한 상황에서, 귀하와 귀하의 LLM은 이 딜레마를 해결하고 학생의 기억력을 희생하지 않으면서 학습을 향상시킬 방법을 찾기 위한 마스터 플랜을 작성해야 합니다. 고려해야 할 질문들: 창의적인 아이디어는 무엇입니까? 그 아이디어들은 현실적입니까? 하나의 아이디어가 있다면, 구체적인 실행 항목은 무엇입니까? AI를 더 많이 사용하고 싶어하는 학생들의 바람을 충족시키면서 부모들의 우려를 어떻게 해결할 수 있습니까? 교사의 역할은 무엇입니까? 수학, 과학, 영어 과목에서 AI 기기의 사용 방식이 달라야 합니까? 어떻게 다르게 접근해야 합니까? 학교 정책은 어떻게 해야 합니까?
""",

    # ── Task content (Noble Industries — Korean) ──────────────────────────────
    "task_noble_md": """\
참여해 주셔서 감사합니다. 아래 과제 설명을 주의 깊게 읽어 주십시오. 내용을 명확히 이해하기 위해 두 번 읽으시기를 권장합니다. 그런 다음 LLM과 대화를 시작하여 최적의 해결책을 함께 논의하고 협력하십시오. 합의에 도달한 후, 화면 하단의 '완료' 버튼을 클릭하여 세션을 종료하십시오. 귀하의 대화는 연구자들을 위해 저장될 것입니다. 진정으로 좋고 효과적인 해결책을 도출하기 위해 최선을 다해 주십시오.
...
귀하는 1988년 대학을 졸업한 후 콜럼버스 오하이오 사이트에서 주니어 프로그래머로 채용되었습니다. 5년 후 시스템 분석가 직책으로 승진했습니다. 현재 귀하는 ISD의 수석 시스템 분석가입니다. 귀하의 그룹은 연구개발부(RDD)의 애플리케이션 개발 및 소프트웨어 지원을 담당합니다. 귀하에게 직접 보고하는 인원은 시스템 분석가 2명, 애플리케이션 프로그래머 4명, 사무 지원 직원 2명입니다.
...
Bob은 그 후 "기밀"이라고 표시된 봉투를 배포했습니다. 그는 "각 봉투에는 10명의 정보 시스템 직원에 대한 프로필 정보가 들어 있습니다. 봉투를 사무실로 가져가서 프로필과 상사의 의견을 읽은 다음, 해고되어야 한다고 생각하는 순서대로 직원들을 순위를 매기십시오. 순위의 이유도 적어 두십시오. 직원들의 실명은 프로필 페이지에 나타나지 않습니다. 왜냐하면 아는 사람에 대해 결정을 내려야 하는 상황을 원하지 않기 때문입니다. 이들은 모두 성과가 좋은 직원들이기 때문에, 경영진은 최종 결정에 대한 입력으로 공정한 기술 동료 그룹의 평가를 원했습니다. 최선의 판단을 사용하십시오. 그런 다음 오늘 나중에 그룹으로 모여 개별 순위를 논의하고 오늘 말까지 최종 순위와 순위 이유를 저에게 제출하십시오." 라고 말했습니다.

Bob은 "이것이 쉬운 일이 아니라는 것을 알고 있지만, 현재 상황을 감안할 때 다른 선택이 없습니다." 라고 말하며 회의를 마쳤습니다. 그 시점에서 귀하와 다른 수석 시스템 분석가들은 순위 지정 작업을 수행하기 위해 각자의 사무실로 돌아갔습니다.
