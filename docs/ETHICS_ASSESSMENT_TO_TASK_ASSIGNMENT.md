...
**Revision note**: This is revision 2 of this document. Revision 1 described task assignment as a plain, personality-independent random draw. It has been superseded — see [Revision History](#revision-history) at the bottom for why.

---

## Summary

After a participant finishes the personality questionnaire, the system (1) scores their responses, (2) classifies them into one of four fixed personality types using a deterministic rule, and (3) saves that result. It then pairs the participant with a task and an AI conversational personality. The **AI-personality** draw is a plain, uniform random choice, made independently of the participant's personality type. The **task** draw works differently by design: to keep the study's four-personality-type × two-task structure from becoming badly imbalanced by chance, the system looks at the participant's already-computed Gerlach type and assigns whichever task is currently under-represented for that type — a standard trial-design technique sometimes called "minimization" or covariate-adaptive randomization. Whenever both tasks are equally represented so far for that type, the choice is still made at random, so it remains unpredictable in advance; it is simply no longer statistically independent of personality type, and that dependency is intentional. No researcher makes or reviews any individual pairing in the normal flow. The participant is not shown their personality type, the AI personality label, or the fact that a second task existed.
...
This is the step most relevant to ethical review of the pairing process. The two draws are deliberately **not** symmetric with each other — one is fully independent random, the other is balanced by design — so they are described separately below.

### 6a. AI conversational personality — plain uniform random, independent of everything

- `assigned_personality` is drawn uniformly at random among the four AI conversational personalities the LLM will adopt during the dialogue (`average`, `role_model`, `self_centred`, `reserved`), using Python's standard-library `random.choice()`.
- This draw reads nothing about the participant — not their Gerlach type, not their task, not any assessment data. It is functionally independent of Step 3's classification by construction.
- *Source: `agent_research_app.py:529` (inside `_get_or_assign`)*

### 6b. Task — balanced (covariate-adaptive) random assignment, not independent by design

- **Why not plain independent randomization.** With four Gerlach types and two tasks, a purely independent 50/50 coin-flip for each participant's task can — purely by chance, especially at this study's target scale (~100 participants split unevenly across four types) — produce badly unbalanced type × task cells (e.g. far more "Self-Centred" participants doing one task than the other). That imbalance would weaken later statistical comparisons across types and tasks, which is the reason this was reworked.
- **Mechanism (`_assign_balanced_task`).** By the time this runs, the participant's Gerlach type is already known and saved (Steps 3–4). The system counts, among all previously-assigned participants who share that same Gerlach type, how many were given each task, and assigns whichever task currently has the lower count *for that type*. If both tasks are equally represented so far for that type, the task is still chosen uniformly at random between them — the outcome is never deterministic, so it remains unpredictable in advance for any single participant.
- **What this is, technically.** This is a standard, widely used trial-design technique sometimes called "minimization" or covariate-adaptive randomization: a known participant characteristic is deliberately used to balance group sizes across strata, while keeping the immediate assignment a coin-flip whenever the running counts are tied. It is a departure from pure independent randomization by design, not an accidental bias — its explicit purpose is to counteract small-sample imbalance, which independent randomization alone cannot guarantee against.
- **The participant still cannot influence or predict their own assignment.** Nothing about a participant's behavior, responses, or preferences feeds into this decision beyond the fixed Gerlach type computed once from the standard IPIP-50 questionnaire in Step 3 — before the participant has any awareness that task assignment is happening at all.
- **Known limitation — honestly stated.** Gerlach type is intrinsic to each participant; it cannot be assigned or balanced by this mechanism. This algorithm can only balance the task split *within* each type as participants arrive — it cannot force the *overall* number of participants per type to be equal. That side of the four-way balance depends on recruitment, which the research team monitors via the admin dashboard's live per-type participant counts (a separate, password-protected internal tool — see below).
- **Not concurrency-safe against simultaneous completions.** The count is computed by reading current on-disk data at assignment time; two participants finishing their assessment at the exact same instant could theoretically both be counted against the same "before" state. At this study's expected pace (participants arriving over days/weeks, not simultaneously), this is a documented but practically negligible limitation, not a hidden one.
- *Source: `agent_research_app.py:427-442` (`_load_gerlach_type`), `:445-480` (`_task_counts_for_type`), `:483-509` (`_assign_balanced_task`)*

### 6c. Applies to both draws
...
- *Source: `agent_research_app.py:512-532` (`_get_or_assign`)*
...
- *Source: `agent_research_app.py:528-530`; `agents/data_models.py:26-57`*
...
- *Source: `agent_research_app.py:535-576` (`render_task_selection`)*
...
- An admin dashboard exists separately from the participant-facing app, is password-protected, and is not reachable by participants. It lets the research team view aggregate counts (including a live balance check across personality types and tasks) and export data for analysis. **No assignment can be created, edited, or reassigned from this dashboard** — the only session-level control available is manually moving a stuck session's stage marker for troubleshooting, which does not touch `assigned_task` or `assigned_personality`.
...
- **Revision 2** (this version, 2026-08-03): Task assignment was reworked to strive for balance across the four Gerlach types × two tasks, since pure independent randomization offered no protection against chance imbalance at this study's scale. The new mechanism (`_assign_balanced_task`, Step 6b above) deliberately reads the participant's already-computed Gerlach type to keep each type's task split as even as possible, while still making an unpredictable random choice whenever counts are tied. This document was revised alongside the code change so that it continues to accurately describe actual system behavior rather than a superseded version of it.
